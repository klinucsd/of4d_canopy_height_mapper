...
def _get_nlcd_clean_mask(lons, lats, bbox, threshold_pct=10):
    """Return boolean array (True = clean) via NLCD 2021 WMS (CONUS only)."""
    min_lon, min_lat, max_lon, max_lat = bbox
    buf = 0.05
...
def _get_worldcover_raster_reprojected(bbox, h, w, dst_transform, dst_crs):
    """Download WorldCover tile and reproject to match target raster grid."""
    catalog = pystac_client.Client.open(
        'https://planetarycomputer.microsoft.com/api/stac/v1',
        modifier=planetary_computer.sign_inplace)
    search = catalog.search(collections=['esa-worldcover'], bbox=bbox)
...
def train_model(X, y, names, model_type='RFR'):
    """
    ...
    ----------
    model_type : str
        'RFR' — Random Forest Regressor (default)
        'HGB' — Histogram Gradient Boosting Regressor
    """
    print("\n" + "="*60)
    ...
    if model_type == 'RFR':
        ...
        raise ValueError(f"Unsupported model_type '{model_type}'. Use 'RFR' or 'HGB'.")
...
def predict_map(model, s2_tif, s1_tif, topo_tif, bbox, output='canopy_height.tif'):
    ...
    with rasterio.open(s2_tif) as s2_src:
        ...
        if s1_tif and os.path.exists(s1_tif):
            with rasterio.open(s1_tif) as s1_src:
                s1_data = np.zeros((s1_src.count, h, w))
            ...
            s1_data = np.array([]).reshape(0, h, w)

        with rasterio.open(topo_tif) as topo_src:
            topo_data = np.zeros((topo_src.count, h, w))
    ...
    wc_data = _get_worldcover_raster_reprojected(bbox, h, w, s2_tfm, s2_crs)
...
if __name__ == "__main__":
    ...
    X, y, features, n_total, n_clean, label_filter = extract_features(
        gedi_csv, s2_path, s1_path, topo_path, bbox)
    print(f"  Label filter: {label_filter}  |  clean: {n_clean:,}/{n_total:,}")
    model = train_model(X, y, features)

    # STEP 4: Predict
    print("\n\nSTEP 4: Prediction\n")

    predict_map(model, s2_path, s1_path, topo_path, bbox,
