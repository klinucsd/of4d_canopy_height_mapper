...
def plot_sentinel2_composite(s2_tif, bbox=None, output_dir=None):
    ...
    with rasterio.open(s2_tif) as src:
        ...
        # Check band count to determine which bands we have
        band_count = data.shape[0]

    fig, axes = plt.subplots(1, 3, figsize=(18, 6))

    # Band indices depend on whether we have 4 bands (old) or 10 bands (new) + NDVI
    if band_count >= 11:  # New: 10 S2 bands + NDVI
        # B02=0, B03=1, B04=2, B05=3, B06=4, B07=5, B08=6, B8A=7, B11=8, B12=9, NDVI=10
        B04_idx, B08_idx, B11_idx = 2, 6, 8
        B02_idx, B03_idx = 0, 1
        ndvi_idx = 10
        band_version = "10-band"
    else:  # Old: 4 S2 bands + NDVI
        # B04=0, B08=1, B11=2, B12=3, NDVI=4
        B04_idx, B08_idx, B11_idx = 0, 1, 2
        B02_idx, B03_idx = 0, 1  # Not available, use B04/B08 as fallback
        ndvi_idx = 4
        band_version = "4-band"

    # RGB composite (Natural color: B04=Red, B03=Green, B02=Blue)
    # For old version, we don't have B02/B03, so use B04/B08/B11 for visualization
    if band_count >= 3:
        if band_count >= 11:
            # New: True natural color (B04, B03, B02)
            rgb = np.stack([data[B04_idx], data[B03_idx], data[B02_idx]])
            rgb_title = 'RGB Composite (Red, Green, Blue)'
        else:
            # Old: Approximation (B04, B08, B11)
            rgb = np.stack([data[B04_idx], data[B08_idx], data[B11_idx]])
            rgb_title = 'RGB Composite (Red, NIR, SWIR1 - 4 band version)'
        ...
        # False color NIR (NIR, Red, Green)
        if band_count >= 11:
            nir_rgb = np.stack([data[B08_idx], data[B04_idx], data[B03_idx]])
        else:
            nir_rgb = np.stack([data[B08_idx], data[B04_idx], data[B11_idx]])

        for i, band in enumerate(nir_rgb):
            nir_rgb[i] = np.clip(band / 0.5, 0, 1)

        axes[1].imshow(np.moveaxis(nir_rgb, 0, -1))
        axes[1].set_title(f'False Color (NIR, Red, Green) - {band_version}')

        # NDVI
        if band_count > ndvi_idx:
            ndvi = data[ndvi_idx]
            ...
            ndvi = (data[B08_idx] - data[B04_idx]) / (data[B08_idx] + data[B04_idx] + 1e-10)
            im = axes[2].imshow(ndvi, cmap='RdYlGn', vmin=-0.2, vmax=1)
...
def create_pipeline_summary(gedi_csv, s2_tif, height_map_path, model_stats,
    ...
    with rasterio.open(s2_tif) as src:
        s2_data = src.read()
        # Handle both 4-band (old) and 10-band (new) versions
        band_count = s2_data.shape[0]
        if band_count >= 11:  # New: 10 S2 bands + NDVI
            ndvi = s2_data[10]  # NDVI is at index 10
        elif band_count >= 5:  # Old: 4 S2 bands + NDVI
            ndvi = s2_data[4]  # NDVI is at index 4
        else:  # Calculate NDVI from NIR and Red
            if band_count >= 11:
                nir_idx, red_idx = 6, 2  # B08, B04
            else:
                nir_idx, red_idx = 1, 0  # B08, B04
            ndvi = (s2_data[nir_idx] - s2_data[red_idx]) / (s2_data[nir_idx] + s2_data[red_idx] + 1e-10)
